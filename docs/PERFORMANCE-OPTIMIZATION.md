...
| 指标 | 可达成 | 难达成 |
|------|--------|--------|
| 3 秒内看到页面结构、卡片文字、占位光球 | ✅ 已接近 | — |
| 3 秒内首屏 **全部** 高清图加载完 | — | ❌ 受签名 RTT 限制 |
| 滚动时无明显卡顿 | ✅ 懒加载 + Masonry 防抖 | — |
| 刷新后比首次更快 | 部分 ✅ 会话签图缓存 | 全量需 IndexedDB |

**让用户「流畅舒适」比死磕 3 秒全图更重要**：先 instant shell，再 progressive image。详见 `CARD-LOADING.md`。
