...
- **本轮变更**：Prompt-first 卡片库首页、黑屏缓存恢复、公开图片模型目录投影已上线；卡片库瀑布流后续已从「固定 `4/3` 媒体框的 CSS multi-column」改为「JS 列容器 + 媒体框走图片真实比例」（`legacy/script/part-03.js`、`styles/base/part-09.css`）。不新增数据库迁移。
