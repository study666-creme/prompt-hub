...
2026-08-06 `20260806a` 候选已通过 Worker `npm run typecheck`、55 个测试文件共 361 项测试、36 份文档检查和完整前端预部署冒烟。新增配置守卫拒绝把 New API 再次绑定到裸 IP 或 `sslip.io`，原有生成幂等、未知结果退款和图片交付回归保持通过。本任务没有发送付费生成 POST；跨任务的单次图片/视频生产验收由统一协调任务消费额度，任何失败或超时都不得在本任务重提。
