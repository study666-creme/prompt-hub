...
- **当前生产**：`20260803a` 已上线，Worker、Pages 与五项既有数据库迁移状态已取证；实时版本继续以 `/health.buildSha` 和 Pages build 为准。
- **本轮已发布**：修复 New API 生图队列提交、完成态无图片引用、列表误载原图、失败媒体黑框、卡片库 Masonry 大空洞、生图分页说明条隔断图片和首屏缩略图优先级。当前公开模型目录只投影活动路由，未确认的历史 Apimart MJ 不再显示为可付费选项。
...
- **Git 与验证已完成**：`20260803a` 从干净审查提交发布；最终运行提交始终从线上 `/health.buildSha` 和 Pages production deployment 读取并核对。
...
2026-08-03 `20260803a` 已通过 Worker `npm run typecheck`、53 个测试文件共 356 项测试，以及卡片库稳定 Grid、失败媒体折叠、最近生图 `_grid` 解析、失效签名重试、首屏优先级、列表状态保留和分页页脚顺序浏览器回归；Worker/Pages 已上线，health、模型目录、商品别名和生产 bundle smoke 均通过。公开 `completed` 必须携带可交付 `imageUrl`；上游先完成后出 URL 时继续确认，无法恢复的历史任务走幂等失败退款。真实付费生图闭环仍需隔离账号执行，不能用重复付费或手工补积分代替。
