...
- **当前发布目标**：`20260806a`，Worker、Pages 与五项既有数据库迁移状态仍分别以 `/health.buildSha`、Pages build 和生产核验为准。
...
- **Git 与验证状态**：`20260806a` 只改 New API 稳定服务入口和对应验证/文档；最终运行提交仍必须从线上 `/health.buildSha` 读取并核对，生产目录必须恢复为非 stale。
...
2026-08-06 `20260806a` 候选已通过 Worker `npm run typecheck`、55 个测试文件共 362 项测试、36 份文档检查和完整前端预部署冒烟。新增配置守卫拒绝把 New API 再次绑定到裸 IP 或 `sslip.io`，模型目录响应恢复版本与 stale 状态监控，原有生成幂等、未知结果退款和图片交付回归保持通过。本任务没有发送付费生成 POST；跨任务的单次图片/视频生产验收由统一协调任务消费额度，任何失败或超时都不得在本任务重提。
