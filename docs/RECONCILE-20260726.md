...
- **当前发布候选**：`20260803a` 已冻结，修复 New API 生图队列提交、完成态无图片引用、列表误载原图、失败媒体黑框、卡片库 Masonry 大空洞、生图分页说明条隔断图片和首屏缩略图优先级。生产切换仍须以最终干净 SHA 的 `/health.buildSha`、Pages build 和线上冒烟取证。
...
2026-08-03 `20260803a` 发布候选已通过 Worker `npm run typecheck`、53 个测试文件共 356 项测试，以及卡片库稳定 Grid、失败媒体折叠、最近生图 `_grid` 解析、失效签名重试、首屏优先级、列表状态保留和分页页脚顺序浏览器回归。公开 `completed` 必须携带可交付 `imageUrl`；上游先完成后出 URL 时继续确认，无法恢复的历史任务走幂等失败退款。根目录文档/预部署与 Pages 暂存检查必须从最终提交再次全量运行；发布后仍需验证 `/health.buildSha`、Pages production build、模型目录 HTTP 200 和一次隔离账号低价生图闭环。
