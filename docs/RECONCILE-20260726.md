...
- **当前发布目标**：`20260804b`，Worker、Pages 与五项既有数据库迁移状态仍分别以 `/health.buildSha`、Pages build 和生产核验为准。
- **本轮发布**：在 `20260804a` 的文字目录收敛之上，三个 MJ 公开号统一从 New API 实时目录进入卡片仓库，新任务不再直连历史 provider；Prompt Hub 不新增数据库迁移，也不改 Pages。
...
- **New API 已核验**：2026-08-04 公开目录返回 `capability_version: 2026-08-04.2`；`mj-v81`、`mj-v7`、`mj-niji7` 均为 0.4 元 / 40 积分/次、固定 Relax 和五图输出，公开目录与定价无内部渠道、域名或映射字段。
...
- **Git 与验证状态**：`20260804a` 已由线上 `/health.buildSha` 核对；`20260804b` 候选保留其全部能力并修正 MJ 动态目录解析，最终运行提交仍必须从线上 `/health.buildSha` 读取并核对。
...
2026-08-04 `20260804b` 候选已通过 Worker `npm run typecheck`、54 个测试文件共 360 项测试和 36 份文档检查，并保留卡片库稳定 Grid、失败媒体折叠、最近生图 `_grid` 解析、失效签名重试、首屏优先级、列表状态保留和分页页脚顺序回归。公开 `completed` 必须携带可交付 `imageUrl`；上游先完成后出 URL 时继续确认，无法恢复的历史任务走幂等失败退款。真实付费生图闭环只允许按本轮明确的单次 40 积分额度执行，失败或超时不得重提。
