...
卡片库提示词首页（2026-08-28）已上线：模型菜单以公开 `GET /api/v1/generate/models` 为唯一实时来源，缓存契约版本为 `20`；首页缓存自愈、卡片库聚焦和 CSS multi-column 列式瀑布流（固定 `4/3` 媒体框防止图片加载时列重平衡闪动）均已进入 Pages。生产生成请求仍使用当前页面配置的同源 Worker。
