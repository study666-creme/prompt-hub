...
- 已发布的 `mj-v82` 和 `Midjourney v8.2 高速` 使用 API 站的
  ...
  保留已有任务的兼容恢复路径；新请求仍按公开模型目录及其标准接口处理。
...
卡片库提示词首页（2026-08-28）已上线：模型菜单以公开 `GET /api/v1/generate/models` 为唯一实时来源，缓存契约版本为 `20`；首页缓存自愈、卡片库聚焦和列容器瀑布流（`.warehouse-focus-columns`，媒体框走图片真实比例，已弃用 CSS multi-column 与固定 `4/3` 媒体框）均已进入 Pages。生产生成请求仍使用当前页面配置的同源 Worker。
