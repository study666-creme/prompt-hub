...
最后核对：2026-07-28。除非明确写“生产”，本文描述的是冻结中的主树候选契约。
...
2. Worker 先按同一可信目录重算并核对 `quotedCredits`，确认一致后才以请求键创建任务并幂等扣费，再把 New API 提交写入持久队列；队列投递不确定时保留数据库 outbox，由 cron 补投队列消息。
3. 队列消费只允许从 `queued` 原子领取一次。上游 HTTP 不确定、`running`、`outcome_unknown` 或任务查询 `not_found` 都不得重新发起付费 POST。
4. 页面按秒轮询；服务端 cron 每 2 分钟兜底推进 submit、poll 和 archive。上游返回临时图后先把任务标为完成并立即给客户端展示，R2/Storage 归档失败只重试本地归档，不重做生成。
