...
复核日期：2026-08-03。下述仓库 UI 对应当前生图媒体可靠性候选；生产是否已切换以线上 build 和资源 HTTP 冒烟为准。
...
6. 失败的近期生成/生图仓库媒体保留原记录和媒体区域，改为显示明确的“图片暂时无法显示”状态，并提供“重新加载”和“删除”操作；重新加载会清理该图的短期失败标记并强制获取新签名。只有现有权威 404/410 清理路径可以移除确实不存在的近期记录，其他错误不得静默删卡或留下黑色方块。
