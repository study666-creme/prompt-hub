...
- **修复的 P1**：公告「我知道了」的已读记录存在 `user_data.data.announcements_seen`，
  被客户端整包上传 `user_data.data` 周期性抹掉，导致公告每次刷新/同步后重新弹出并
  遮挡全站（登录用户每分钟可复现）。已改为 KV（`PROMPT_HUB_METRICS`，键
  `ann_seen:{userId}`，40 天 TTL），GET 兼容读取旧 user_data 记录，KV 缺失回退旧行为；
...
- 主功能走查均正常：注册即登录、卡片新建/编辑/标签/搜索、每日 5 积分签到、
  电脑登录领 1 天基础会员、社区 feed（`community/sign` 300–450ms，无历史回源慢问题）、
  移动端卡片网格（需按移动端尺寸首载；桌面中途缩窗口不触发重排，刷新即恢复）。
- 遗留观察：生图完成后「最近」列表在本测试环境未自动刷新、手动「获取」/切页签正常
  （环境 rAF 冻结无法完全排除客户端原因，真实浏览器待观察）；手机验证未开放导致
  邀请码兑换不可用（产品状态非缺陷）；签到面板「已连续签到 0 天」与 toast「连续签到
  1 天」口径不一致（文案级）。
...
## 2026-09-13 公告管理无法保存：两层根因叠加（已修复上线）

- **前端层**：save(silent) 在 editingIndex 残留时仍读隐藏编辑器——空文本 return（请求不发）
  或旧快照覆盖改动。修复：静默保存彻底不碰编辑器 + editingIndex 保存/取消后复位（3d71e75，
  附 11 场景 DOM 桩回归，旧代码可复现）。Pages 构建 20260913c/d。
- **CORS 层（真正从未通过的一层）**：Worker allowMethods 缺 PUT——PUT /api/admin/announcements
  从上线起就被浏览器预检拦截，前端报「无法连接 API」。修复 allowMethods 加 PUT（25b1b78），
  Worker 版本 d11e1197，线上 OPTIONS 已验证返回 PUT。
- 教训：管理端改动必须用带真实 fetch 的端到端验证（本次前端桩测试 + 线上 OPTIONS 探针），
  「页面能开、列表能读」不代表保存链路通。
