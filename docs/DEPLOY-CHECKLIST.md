...
生产基线仍为 `20260730a`。本候选 build `20260803a` 在既有 durable queue 与失败媒体折叠基础上，补齐 `image2-A` 的画质/分辨率价格矩阵、全能模型选择器隔离、异步结果轮询及失效最近图片的重载/删除操作；代码、测试与文档已完成，待同一干净提交发布后以 `/health.buildSha` 和 Pages production build 核对。
