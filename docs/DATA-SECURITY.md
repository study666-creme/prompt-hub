...
| 图片/视频任务 | 只读本人 | 扣费、结算、退款和归档 |

RLS 和 GRANT 定义在 `supabase/`。恢复新库后必须执行 schema/迁移并用 `/health` 验证 service role，不要临时开放全表匿名读写来绕过权限错误。

付费视频操作必须使用数据库 RPC `consume_user_credits` 与 `refund_user_credits`。请求 ID、任务 ID和退款原因共同形成稳定幂等边界；不得用客户端重试、直接更新 `profiles` 或非原子 `quota += N` 代替。跨过上游 POST 边界但无法确认结果时保留任务栅栏，不退款也不重放，直到明确终态或 1 小时 SLA 到期。
