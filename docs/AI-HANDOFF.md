...
卡片库提示词首页（2026-08-28）已上线：`warehouse-composer.js` 直接保存卡片到现有仓库，并在同页内联卡片库、社区和生成记录。卡片库工具条常驻文件分类、标签分类、搜索和排序；首个明确向下滚轮手势收起输入区与左侧导航，顶部向上滚动恢复。生图菜单只消费公开实时目录，目录缓存版本为 `20`，不把 `generic` 模型归入全能模型2；聚焦后的卡片库使用列式瀑布流并保留快速预览。
