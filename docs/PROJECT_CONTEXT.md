...
## 当前部署阶段（2026-06-01 · 构建 **20260601j**）

| 项 | 状态 |
|----|------|
| Worker | `prompt-hub-api` · API https://api.prompt-hub.cn |
| Pages | https://prompt-hub.cn |
| 构建号 | **20260601j**（主站）· 改静态后需 **Pages + Worker** 同发 |
| **批量生图** | 先慢后快轮询；同任务多图补全；列表 jobs 限流同步 |
| **加载动效** | 生成中/加载中 = 加载完成同一光带 `card-beam-trbl` → `card-beam-loading` 循环 |
| **卡片创作 / 社区** | 缩略图预取、批量签名等同前 |

### 已知问题 / 下一步

- 强刷确认 `window.__APP_BUILD__ === '20260601j'`
