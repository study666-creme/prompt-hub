  ...
  it('builds every published canvas video model from its declared wire fields', () => {
    ...
    const cases: Array<{
    ...
    }> = [
      {
        ...
        expected: { model: 'minimax_h3', prompt: 'animate', seconds: 4, size: '768', images: refs.referenceImages, reference_videos: refs.referenceVideos, reference_audios: refs.referenceAudios }
      },
      ...
      ...['S-videos-t-431-pro-720-5', 'S-videos-t-431-fast-720-5'].map(model => ({
        ...
        expected: { model, prompt: 'animate', seconds: 4, resolution: '720P', aspect_ratio: '16:9', reference_images: refs.referenceImages, video_references: refs.referenceVideos, audio_reference: refs.referenceAudios }
      })),
      {
        model: 'runway-gen4.5',
        duration: 5,
        ...
        expected: { model: 'runway-gen4.5', prompt: 'animate', seconds: 5, size: '1280x720', images: refs.referenceImages }
      },
      {
        model: 'kling-o3',
        resolution: '1280x720',
        parameters: native('kling-o3', field('referenceImages', 'array', { path: 'reference_images', max_items: 6 }), field('referenceVideos', 'string', { path: 'input_video' })),
        expected: { model: 'kling-o3', prompt: 'animate', seconds: 4, size: '1280x720', reference_images: refs.referenceImages, input_video: refs.referenceVideos[0] }
      },
      {
        model: 'kling-o3-pro-v2v-reference',
        resolution: '1280x720',
        parameters: native('kling-o3-pro-v2v-reference', field('referenceImages', 'array', { path: 'images', max_items: 2 }), field('referenceVideos', 'string', { path: 'input_video' })),
        expected: { model: 'kling-o3-pro-v2v-reference', prompt: 'animate', seconds: 4, size: '1280x720', images: refs.referenceImages, input_video: refs.referenceVideos[0] }
      },
      {
        model: 'kling-v3',
        resolution: '1280x720',
        parameters: native('kling-v3', field('referenceImages', 'array', { path: 'images', max_items: 2 })),
        expected: { model: 'kling-v3', prompt: 'animate', seconds: 4, size: '1280x720', images: refs.referenceImages }
      },
      {
        model: 'kling-v3-omni-v2v-create',
        resolution: '1280x720',
        parameters: native('kling-v3-omni-v2v-create', field('referenceImages', 'array', { path: 'style_references', max_items: 3 }), field('referenceVideos', 'string', { path: 'input_video' })),
        expected: { model: 'kling-v3-omni-v2v-create', prompt: 'animate', seconds: 4, size: '1280x720', style_references: refs.referenceImages, input_video: refs.referenceVideos[0] }
      },
      ...['veo-3.1', 'veo-3.1-fast', 'veo-3.1-lite'].map(model => ({
        ...
        expected: { model, prompt: 'animate', seconds: model === 'veo-3.1-fast' ? 8 : 4, size: '1280x720', images: refs.referenceImages }
      })),
      ...['S-videos-f-933-pro-3', 'S-videos-f-933-fast-3'].map(model => ({
        model,
        resolution: '720p',
        parameters: legacy(model, true),
        expected: { model, prompt: 'animate', duration: 15, ratio: '16:9', resolution: '720p', referenceImages: refs.referenceImages, referenceVideos: refs.referenceVideos, referenceAudios: refs.referenceAudios }
      }))
    ];
