  ...
  function init() {
    ...
    onAppChange(window.AppRouter?.resolveBootApp?.() || 'landing');
  }
  ...
  function initImageGenForm() {
    ...
    if (draft) {
      ...
      if (qEl && draft.quality) qEl.value = normalizeImageGenQualityOptionValue(draft.quality);
      if (draft.model) {
  ...
  function getImageGenQuality() {
    return document.getElementById('imageGenQuality')?.value || 'medium';
  }
  ...
  const IMAGE_GEN_MODEL_FALLBACK = [
    ...
    { id: 'sensenova-1.5-一秒出图', label: '一秒出图 1K', uiFamily: 'generic', sortOrder: 60, selectable: true, status: 'active', refundOnViolation: true, resolutions: ['1k'], aspectRatios: ['1:1', '16:9', '9:16', '4:3', '3:4'] },
    { id: 'seedream-5.0', label: 'Seedream 5.0 Pro 1K/2K', uiFamily: 'generic', sortOrder: 61, selectable: true, status: 'active', refundOnViolation: true, resolutions: ['1k', '2k'], aspectRatios: ['1:1', '4:5', '2:3', '3:2', '9:16', '16:9'] },
  ...
  function sanitizeImageGenModelDescription(description) {
    ...
    s = s.replace(/^(OpenAI|Gemini|备用线路)\s*[·•]\s*/gi, '');
    s = s.replace(/\b(OpenAI|Gemini|gpt-image-2|official|备用线路)\b\s*[·•]?\s*/gi, '');
  ...
  function normalizeImageGenModelEntry(m) {
    const publicInput = {
      ...
      creditsByResolution: m?.creditsByResolution,
      pricingBySpeed: m?.pricingBySpeed,
      creditsBySpeed: m?.creditsBySpeed,
      creditsPerCall: m?.creditsPerCall,
      creditsFinal: m?.creditsFinal
    };
    ...
    return {
      ...
      catalogLabel,
      description,
      uiFamily: projected.uiFamily,
      sortOrder: projected.sortOrder,
      status: projected.status,
      selectable: projected.selectable,
      refundOnViolation: projected.refundOnViolation,
      statusNotice: projected.statusNotice,
      violationNotice: projected.violationNotice,
      fixedQualityLow: projected.fixedQualityLow,
      maxReferenceImages: projected.maxReferenceImages,
      resolutions: projected.resolutions,
      aspectRatios: projected.aspectRatios,
      parameters: projected.parameters,
      pricingByResolution: projected.pricingByResolution,
      creditsByResolution: projected.creditsByResolution,
      pricingBySpeed: projected.pricingBySpeed,
      creditsBySpeed: projected.creditsBySpeed,
      creditsPerCall: projected.creditsPerCall,
      creditsFinal: projected.creditsFinal
    };
  }
