...
const IMAGEGEN_FEED_MIN_CARD_PX = 72;
  ...
  let deps = {};
  const feedCards = global.ImageGenFeedCards.create({
    getDeps: () => deps,
    feedImgStorageAttr,
    resolveFeedCardDisplay
  });
  ...
  function resolveImageGenFeedScrollRoot(wrap) {
    ...
    function readFeedCardRefImages(card) {
      return feedCards.readFeedCardRefImages(card);
    }

    function buildFeedCardHtml(opts) {
      return feedCards.buildFeedCardHtml(opts);
    }
    ...
    function warehouseCardToFeedHtml(c) {
      return feedCards.warehouseCardToFeedHtml(c);
    }

    function communityPostToFeedHtml(p) {
      return feedCards.communityPostToFeedHtml(p);
    }
    ...
    function bindImageGenFeedCardEvents(wrap, cardsOnly) {
      ...
      cardList.filter((card) => !card.dataset.pending && !card.dataset.failed).forEach((card) => {
        ...
        const getFeedRefAssets = () => {
          ...
          const sourceCardId = img?.getAttribute('data-source-card-id') || feedItemId || '';
